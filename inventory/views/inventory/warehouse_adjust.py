...
SESS_ADD_FAILED = 'adjust add failed'
SESS_EDIT_FAILED = 'adjust edit failed'

########                    
# List #
########    
@view_config(route_name='inventory-warehouse-adjust', renderer='templates/warehouse_adjust/list.pt',
             permission='inventory-warehouse-adjust')
...
# Action #
##########    
@view_config(route_name='inventory-warehouse-adjust-act', renderer='json',
             permission='inventory-warehouse-adjust-act')
...
def form_validator(form, value):
    ...
    if 'id' in form.request.matchdict:
        ...
        q = DBSession.query(ProductAdjust).filter_by(id=uid)
        produk = q.first()
    else:
        produk = None
        
    q = DBSession.query(ProductAdjust).filter_by(kode=value['kode'])
    found = q.first()
    if produk:
    ...
    if 'nama' in value: # optional
        found = ProductAdjust.get_by_nama(value['nama'])
        if produk:
            if found and found.id != produk.id:
                err_nama()
        elif found:
...
class AddSchema(colander.Schema):
    ...
    product_accept_kd   = colander.SchemaNode(
                          colander.String(),
                          oid = "product_accept_kd",
                          title ='Penerimaan')
...
@view_config(route_name='inventory-warehouse-adjust-add', renderer='templates/warehouse_adjust/add.pt',
             permission='inventory-warehouse-adjust-add')
def view_add(request):
    ...
    if request.POST:
        if 'simpan' in request.POST:
            ...
            print '......................---------------...........',row			
            return HTTPFound(location=request.route_url('inventory-warehouse-adjust-edit',id=row.id))
...
def query_id(request):
    return DBSession.query(ProductAdjust).filter_by(id=request.matchdict['id'])
...
@view_config(route_name='inventory-warehouse-adjust-edit', renderer='templates/warehouse_adjust/edit.pt',
             permission='inventory-warehouse-adjust-edit')
def view_edit(request):
    ...
    if row.disabled: 
        request.session.flash('Data tidak dapat diedit.', 'error')
        return route_list(request)	
    ...
    if request.POST:
        if 'simpan' in request.POST:
            ...
            try:
                ...
                return HTTPFound(location=request.route_url('inventory-warehouse-adjust-edit',
                                  id=row.id))
...
# Delete #
##########    
@view_config(route_name='inventory-warehouse-adjust-delete', renderer='templates/warehouse_adjust/delete.pt',
             permission='inventory-warehouse-adjust-delete')
